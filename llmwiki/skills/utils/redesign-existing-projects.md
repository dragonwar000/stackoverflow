...
- **Table/list rows that grow taller than their siblings because long text (filename, title, email) wraps unbounded.** Especially visible with `table-layout: fixed` and real (not placeholder) data — a compact paginated table can balloon into thousands of pixels of scroll. Cap the cell to one line with `white-space: nowrap; overflow: hidden; text-overflow: ellipsis;` (add `max-width: 0` if truncation doesn't kick in under `table-layout: fixed`), and carry the full value in a `title` attribute or tooltip so only the layout is protected, not the information.
...
**2. Write** `llmwiki/wiki/draft/uiux/DDMMYY-<ten>.md`:
...
## Origin
- **Draft:** `wiki/draft/uiux/DDMMYY-<ten>.md`
- **Commit:** _(filled by verify-before-commit)_
- **Date promoted:** _(filled by verify-before-commit)_
```

**3. Update wiki index & log:**
- `llmwiki/wiki/index.md` — append one row: `| [DDMMYY-<ten>](draft/uiux/DDMMYY-<ten>.md) | draft | YYYY-MM-DD |`
