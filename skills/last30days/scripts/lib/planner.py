...
# Sources gated behind an explicit user opt-in (INCLUDE_SOURCES). The depth
# trimmer must never drop these from a subquery that requested them - the
# user already paid the opt-in cost, and no priority table ranks them.
OPT_IN_SOURCES = {"voz", "perplexity"}
        ...
        if depth in {"quick", "default"}:
            ...
            if len(preferred_sources) < limit:
                for source in ranked_sources:
                    ...
                    if len(preferred_sources) >= limit:
                        break
        # Opt-in sources reach available_sources only through an explicit
        # user gate (INCLUDE_SOURCES). The priority tables above predate
        # them, so trimming would silently undo the opt-in: keep any the
        # plan put on this subquery.
        for source in subquery.sources:
            if (
                source in OPT_IN_SOURCES
